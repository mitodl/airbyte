...
* `use_aws_default_credential_provider_chain`: Allows use of the [default AWS credential provider chain](https://boto3.amazonaws.com/v1/documentation/api/latest/guide/credentials.html#configuring-credentials). This is useful if you are attempting to use EC2 instance profiles instead of access key and secret access keys. Leave `aws_access_key_id` and `aws_secret_access_key` blank if you set this this to true.
